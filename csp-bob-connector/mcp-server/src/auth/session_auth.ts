...
export class SalesforceSessionAuth {
  private sessionToken: string;
  private tokenSetAt: Date | null;
  ...
  constructor(config?: Partial<SessionAuthConfig>) {
    ...
    if (!token || token.trim() === "") {
      throw new AuthError(
        "CSP_SESSION_TOKEN env var is not set. " +
    ...
    if (!token.includes("!")) {
      throw new AuthError(
        "CSP_SESSION_TOKEN does not look like a valid Salesforce session ID. " +
  ...
  private checkAgeWarning(): void {
    if (!this.tokenSetAt) return;

    const ageMs = Date.now() - this.tokenSetAt.getTime();
    if (ageMs > this.warnAfterMs) {
      const ageHours = Math.round(ageMs / (1000 * 60 * 60));
      console.warn(
        `[SalesforceSessionAuth] WARNING: CSP session token is ~${ageHours}h old. ` +
 ...
 */
export function createSessionAuth(): SalesforceSessionAuth {
